...
module DNA = struct
	...
	let index = function
		...
		| 'G' | 'g' -> 2
		| 'T' | 't' -> 3
		| _ as nuc -> invalid_arg (sprintf "unrecognized DNA nucleotide %c" nuc)
	...
	let of_index = function
		...
		| 2 -> 'G'
		| 3 -> 'T'
		| _ -> invalid_arg "Code.DNA.of_index"
...
module RNA = struct
	...
	let index = function
		...
		| 'G' | 'g' -> 2
		| 'U' | 'u' -> 3
		| _ as nuc -> invalid_arg (sprintf "unrecognized RNA nucleotide %c" nuc)
	...
	let of_index = function
		...
		| 2 -> 'G'
		| 3 -> 'U'
		| _ -> invalid_arg "Code.RNA.of_index"
...
module Codon64 = struct
	...
	let is (nuc1,nuc2,nuc3) = (DNA.is (RNA.to_DNA nuc1) && DNA.is (RNA.to_DNA nuc2)
	                           && DNA.is (RNA.to_DNA nuc3))

	let index (nuc1,nuc2,nuc3) = (16 * (DNA.index (RNA.to_DNA nuc1))
	                              + 4 * (DNA.index (RNA.to_DNA nuc2))
								  + (DNA.index (RNA.to_DNA nuc3)))
...
module Codon61 = struct
	...
	let is c = Codon64.is c && not (Codon64.is_stop c)

	let order = [| ('T','T','T'); ('T','T','C'); ('T','T','A'); ('T','T','G'); ('T','C','T');
	               ('T','C','C'); ('T','C','A'); ('T','C','G'); ('T','A','T'); ('T','A','C');
				   ('T','G','T'); ('T','G','C'); ('T','G','G'); ('C','T','T'); ('C','T','C');
				   ('C','T','A'); ('C','T','G'); ('C','C','T'); ('C','C','C'); ('C','C','A');
				   ('C','C','G'); ('C','A','T'); ('C','A','C'); ('C','A','A'); ('C','A','G');
				   ('C','G','T'); ('C','G','C'); ('C','G','A'); ('C','G','G'); ('A','T','T');
				   ('A','T','C'); ('A','T','A'); ('A','T','G'); ('A','C','T'); ('A','C','C');
				   ('A','C','A'); ('A','C','G'); ('A','A','T'); ('A','A','C'); ('A','A','A');
				   ('A','A','G'); ('A','G','T'); ('A','G','C'); ('A','G','A'); ('A','G','G');
				   ('G','T','T'); ('G','T','C'); ('G','T','A'); ('G','T','G'); ('G','C','T');
				   ('G','C','C'); ('G','C','A'); ('G','C','G'); ('G','A','T'); ('G','A','C');
				   ('G','A','A'); ('G','A','G'); ('G','G','T'); ('G','G','C'); ('G','G','A');
				   ('G','G','G') |]
	let lookup = ord order

	let index (n1,n2,n3) = lookup (Char.uppercase (RNA.to_DNA n1),
	                               Char.uppercase (RNA.to_DNA n2),
								   Char.uppercase (RNA.to_DNA n3))
	let of_index idx = order.(idx)
	
	let compare c1 c2 = compare (index c1) (index c2)
	
	let translate c = Codon64.translate c
